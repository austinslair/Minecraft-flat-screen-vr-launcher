    ...
    EGLBoolean record_present(EGLDisplay dpy,EGLSurface surface,EGLBoolean result){
        ...
        if(enabled && result==EGL_TRUE){
            ...
            if(stats.frame(reinterpret_cast<uintptr_t>(dpy),reinterpret_cast<uintptr_t>(surface),report))
                LOG_V("TGS_FRAME fps=%.2f mean_ms=%.3f p95_ms=%.3f worst_ms=%.3f mapped=%llu fallback=%llu reference=%llu upload_bytes=%llu staged_uploads=%llu staged_bytes=%llu direct_large_uploads=%llu",
                  ...
                  (unsigned long long)report.chunks.staged,(unsigned long long)report.chunks.bytes,(unsigned long long)report.chunks.direct);
        }
        return result;
    }
